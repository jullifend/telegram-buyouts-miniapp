        ...
        for product in PRODUCT_SEED:
            ...
            if not existing:
                db.execute(
                    ...
                    (sku, title, description, image_url, marketplace_url, daily_limit, is_active, sort_order, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?)
                    """,
                    (
                        product["sku"],
...
def update_product(product_id: int, payload: ProductUpdate, _: None = Depends(validate_admin_key)) -> Dict[str, Any]:
    with get_db() as db:
        ...
        db.execute(
            """
            UPDATE products
            SET sku=?, title=?, description=?, image_url=?, marketplace_url=?, daily_limit=?, is_active=?, updated_at=?
            WHERE id=?
            """,
            (
                payload.sku,
